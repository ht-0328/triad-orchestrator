...
```text
## 計画マクロフェーズ（全てAI主体。ここでは人間を止めない）

INTAKE（Codex。解決方法を決めず、短い依頼を調査・協議ブリーフへ整理）
  -> RESEARCH（Antigravity。公式情報・競合・技術制約を調査。明示的なSKIPを許可）
  -> SOLUTION_PROPOSAL（Claude。要求・制約を踏まえて複数の実現案を比較）
  -> PROPOSAL_REVIEW（Codex。独立見解を組み立て、Claude案を批判的にレビュー）
  -> PROPOSAL_FACT_CHECK（Antigravity。外部事実と現行仕様を再確認。明示的なSKIPを許可）
  ...
  -> DESIGN_RESEARCH（Antigravity。設計上の外部前提を実地確認。明示的なSKIPを許可）
  -> DESIGN_REVIEW（Codex）
...
## 成果物マクロフェーズ（計画承認後のビルド・検証。末尾に人間ゲート2）
  ...
  -> E2E_VERIFY（Antigravity。明示的なSKIPを許可）
